...
def simplex_iteration(A, b, C, m: int, n: int,is_auxillary,Answer_dictionary):
    
    print()
    print("..............................................................")
    print("in simplex iteration: ")
    print("A",A)
    print("b",b)
    print("C",C)
    print("m",m)
    print("n",n)
    print("is_auxilarry",is_auxillary)
    ...
    if(is_auxillary!=1):  
      ...
      print(np.round(Down_Table_initial,2))
      print("-----------------------------------------------")
    ...
    while(MaxRC > eps):
      ...
      if (Index_Leave == -1):
        ...
        print(np.round(Down_Table_final,2))
        print("-----------------------------------------------")
    ...
    if(is_auxillary==0):     
      ...
      Answer_dictionary["optimal_value"]=np.round(Z,2)
    else:
      if(Z>0):
...
def simplex_algo():
  Answer_dictionary = {"initial_tableau":np.array([[]]),"final_tableau":np.array([[]]),"solution_status":"","optimal_solution":[],"optimal_value":0}
  is_max = True
  A1 = []
  b1 = []
  constraint_type = []
  C1 = []
  file = open("assignment1\input.txt", "r")
  ...
  C1 = [int(x) for x in content[index_C+1][:-1].split(',')] 
  if(is_max==False):
    C1 = [x * -1 for x in C1]
  A1,b1,C1 = toStandard(A1.copy(),b1.copy(),C1.copy(),constraint_type)  
  var_to_denote_feasible = 1
  C_temp = [0]*(len(A1[0])-len(constraint_type))+[-1]*len(constraint_type)
  A=np.array(A1.copy())
  b=np.array(b1.copy())
  C1 = np.array(C1)
  C3=C_temp.copy()
  C3 = C3+[0]*len(constraint_type)
  C3 = np.array(C3)
  A3=A1.copy()
  ...
  A3 = np.array(A3)
  b3=np.array(b1.copy())
  var_to_denote_feasible = simplex_iteration(A3,b3,C3,len(A3),len(A3[0])-len(A3),var_to_denote_feasible,Answer_dictionary)
  simplex_iteration(A,b1,C1,len(A),len(A[0])-len(A),var_to_denote_feasible,Answer_dictionary)
  if(is_max==False):
